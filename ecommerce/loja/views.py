...
from .models import *
import uuid
from .utils import filtrar_produtos, ordenar_produtos, enviar_email_compra, exportar_csv
...
def loja(request, filtro=None):
    ...
    context = {"produtos": produtos, "banners": banners, "categorias": categorias}
...
def ver_produto(request, id_produto):
    ...
    context = {"produto": produto, "itens_estoque": itens_estoque, "tem_estoque": tem_estoque, "tamanhos": tamanhos, "similares": similares}
...
def finalizar_pedido(request, id_pedido):
    if request.method == "POST":
        ...
        # verificando se o preco no html e o msm do banco de dados
        if total != float(pedido.preco_total):
            erro = "preco"

        # verificando se foi algum endereco selecionado
        ...
        if erro:
            ...
            link = "https://webhook.site/1d7959d9-ae06-415a-9e34-7c63f2865ec8"
    ...
    if request.method == "POST":
        ...
        if "senha_atual" in dados:
            ...
            if nova_senha == nova_senha_confirmacao:
                # verifica se a senha atual esta certa
...
def meus_pedidos(request):
    cliente = request.user.cliente
    pedidos = Pedido.objects.filter(status_pedido=True, cliente=cliente).order_by("-data_finalizacao")
    ...
    if request.method == "POST":
        ...
        if "email" in dados and "senha" in dados and "confirmacao_senha" in dados:
            ...
            if not email:
                ...
                # Só continue se não houver erro de validação
                if not erro:
...
def gerenciar_loja(request):
    if request.user.groups.filter(name="equipe").exists():
        pedidos_finalizados = Pedido.objects.filter(status_pedido=True)
...
def exportar_relatorio(request, relatorio):
    if request.user.groups.filter(name="equipe").exists():
        if relatorio ==  "pedido":
            informacoes = Pedido.objects.filter(status_pedido=True)
