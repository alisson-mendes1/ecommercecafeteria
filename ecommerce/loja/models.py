...
class ItemEstoque(models.Model):
    ...
    tamanho = models.ForeignKey(Tamanho, null=True, blank=True, on_delete=models.SET_NULL) #(300ml, 400ml, 500ml, 750ml, 1000ml)

    def __str__(self):
        return f"Produto: {self.produto}, {self.tamanho}, Quantidade: {self.quantidade}"
...
class Pedido(models.Model):
    ...
    status_pedido = models.BooleanField(default=False) # (finalizado, andamento, pedido criado)
    codigo_transacao = models.CharField(max_length=200, null=True, blank=True)
    endereco = models.ForeignKey(Endereco, null=True, blank=True, on_delete=models.SET_NULL)

    def __str__(self):
        return f"Cliente: {self.cliente.email} - id_pedido: {self.id} - Finalizado: {self.status_pedido}"
...
class Banner(models.Model):
    ...
    def __str__(self):
        return f"{self.link_destino}, Ativo: {self.status_ativo}, Nome imagem: {self.imagem}"